...
client = OpenAI(base_url=endpoint, api_key=foundry)

JUDGES = ["gpt-5.4-mini", "Mistral-Large-3", "grok-4-20-non-reasoning"]
...
def fetch_pending_rag_responses(cur, id_modelo_juiz: int) -> list:
    cur.execute(
        """
        SELECT
            r.id_resposta,
...
def save_avaliacao(cur, id_resposta: int, id_modelo_juiz: int, id_prompt: int, nota: int, justificativa: str):
    cur.execute(
        "INSERT INTO avaliacoes_juiz (id_resposta, id_modelo_juiz, id_prompt, nota, justificativa, rag) "
        "VALUES (%s, %s, %s, %s, %s, TRUE)",
        (id_resposta, id_modelo_juiz, id_prompt, nota, justificativa),
    )


PROMPT_JUIZ_RAG = """[PERSONA]
...
[RETRIEVED CONTEXT]
The AI had access to the following medical documents as supporting context:
{contexto_rag}
...
- Analyze whether the retrieved context helped correct a potential hallucination or introduced noise/confusion into the response. Mention this explicitly in your reasoning.

[OUTPUT INSTRUCTIONS]
Provide the verdict STRICTLY in this format, with no additional text before or after:
REASONING: <detailed technical justification comparing the AI answer to the Gold Standard, noting the impact of the retrieved context>
SCORE: <only a single digit from 1 to 5>"""


def llm_as_judge(question: str, golden: str, model_answer: str, contexto_rag: str, deployment: str) -> dict:
    prompt = PROMPT_JUIZ_RAG.format(
        question=question,
        golden=golden,
        model_answer=model_answer,
        contexto_rag=contexto_rag or "(Nenhum contexto recuperado)",
    )
    ...
    resposta_juiz = completion.choices[0].message.content or ""
    ...
    return {"nota": nota, "chain_of_thought": chain_of_thought}
...
def main():
    ...
    try:
        ...
        id_prompt = ensure_judge_prompt(cur, PROMPT_JUIZ_RAG)

        for judge_model in JUDGES:
            print(f"\n{'='*60}")
            print(f"Juiz (RAG): {judge_model}")
            print(f"{'='*60}")

            id_modelo_juiz = ensure_judge_model(cur, judge_model)

            respostas = fetch_pending_rag_responses(cur, id_modelo_juiz)
            print(f"{len(respostas)} respostas RAG pendentes.")
            ...
            for i, row in enumerate(respostas, 1):
                print(f"  [{i}/{len(respostas)}] Resp {row['id_resposta']} | {row['model_name']} | {row['nome_dataset']}")

                resultado = llm_as_judge(
                    question=row["question"],
                    golden=row["golden"],
                    model_answer=row["model_answer"],
                    contexto_rag=row["contexto_rag"],
        ...
        print("\nTodas as avaliações RAG concluídas.")
